...
public class BakJoon14503 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //// 청소기 보고 잇는 방향 -> 0 북 1 동 2 남 3 서 -> 좌회전이니깐 역순으로 봐줘야됨 ㅠㅠ
        ...
        bfs(new Point(cleanerX, cleanerY, force));
    ...
    static void bfs(Point start) {
        ...
        while (!q.isEmpty()) {
            ...
            for (i = 0; i < 4; i++) {
                int nx = check.x + (dx[(3 - i + check.force) % 4]);
                int ny = check.y + (dy[(3 - i + check.force) % 4]); // 보고 있는 방향을 기준으로 왼쪽부터 차례대로 확인
                if (nx >= 0 && ny >= 0 && nx < width && ny < height && area[nx][ny] == 0) {
                    // 지도안이거나 방문했던 경우 제외 벽 제외 + 청소할 곳을 찾음
                    q.add(new Point(nx, ny, (3 - i + check.force) % 4));
                    count++; // 그곳으로 가서 청소
                    area[nx][ny] = -1;
                    continue Out;
                }
            ...
            // 청소할 곳을 못찾음!
            if (area[check.x + dx[(check.force + 2) % 4]][check.y + dy[(check.force + 2) % 4]] == 1) { // 뒤쪽이 벽
                break;
            } // 뒤쪽이 벽 아님 -> 후진
            q.add(new Point(check.x + dx[(check.force + 2) % 4], check.y + dy[(check.force + 2) % 4], check.force));
        }
    }
