...
또한 파일, 네트워크 등으로 출력되어 "전송"될 수도 있다.
...
- 매개값으로 주어진 바이트 배열의 길이만큼 바이트를 읽고 배열에 저장한고 읽은 바이트 수를 리턴한다.

- 입력 스트림에서 5개의 바이트가 들어온다면 길이 3인 바이트 배열로 두 번 읽을 수 있다.
...
-> read() 메소드가 리턴한 int값을 char타입으로 변환하면 읽은 문자를 얻을 수 있따.
...
=> System.in으로 키보드에서 a를 입력하고 Enter키를 눌렀다면 a키의 71, enter의 13,10이 읽혀진다.
...
public class SystemOutExample {
	public static void main(String[] args) throws Exception {
		...
		os.write(10); // 아스키 코드 48~57까지의 문자
		
		for(byte b=97; b<123; b++) {
			os.write(b);
		}		
		os.write(10); // 아스키 코드 97~122까지의 문자
...
- 파일 크기, 파일 속성, 파일 이름등의 정보를 얻어는 기능
